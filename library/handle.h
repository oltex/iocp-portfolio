...
namespace library {
	...
	public:
		...
		inline static auto wait_for_multiple(library::span<type> span, bool const wait_all, unsigned long const milli_second) noexcept -> unsigned long {
			HANDLE handle[MAXIMUM_WAIT_OBJECTS];
			for (auto index = 0u; index < span.size(); ++index)
				handle[index] = span[index].data();
			return ::WaitForMultipleObjects(static_cast<unsigned long>(span.size()), handle, wait_all, milli_second);
		}
